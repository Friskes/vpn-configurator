...
- **WireGuard для Android** — чёрный или белый список приложений по именам Android-пакетов (`ExcludedApplications` / `IncludedApplications`), плюс галочка «Не пускать RustDesk в туннель», добавляющая пакет `com.carriez.flutter_hbb`. Эти два ключа понимает только WireGuard для Android и его форки (AmneziaWG, WG Tunnel) — десктопные клиенты откажутся грузить такой конфиг с ошибкой `Invalid key`, поэтому режим вынесен отдельным пунктом.
